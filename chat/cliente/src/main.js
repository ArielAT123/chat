...
socket.on('existing-users', async (users) => {
  users.forEach(user => {
    addParticipant(user.username, false, user.id);
  });
});

socket.on('offer', async ({ offer, from, username }) => {
  addParticipant(username, false, from);
...
function toggleMute() {
  ...
  if (state.isMuted) {
    ...
    if (elements.micIconMain) elements.micIconMain.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="32px" height="32px" viewBox="0 0 24 24" fill="none">
